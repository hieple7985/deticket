...
export const Topbar = () => {
  ...
  const getUserAvatar = (): string => {
    ...
    const userAvatarData = new Identicon(
      activeAccount?.address,
      420
    ).toString();
    return `data:image/png;base64,${userAvatarData}`;
  };
            ...
            <div className="relative flex items-center justify-between h-16">
                      ...
                      <Link
                        ...
                        className={classNames(
                          item.current
                            ...
                            : "text-gray-300 hover:bg-[rgb(28,46,91)] hover:text-white",
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                ...
                <Menu as="div" className="ml-3 relative">
                  <div>
                    {activeAccount && (
                      <Menu.Button className="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800">
                        <span className="sr-only">Open user menu</span>
                        <div className="text-white mr-2">
                          {getShortAddress(activeAccount?.address)}
                        </div>
                        <img
                          className="h-8 w-8 rounded-full"
                          src={getUserAvatar()}
                          alt={`Address ${activeAccount?.address}`}
                        />
                      </Menu.Button>
                    )}
                    {!activeAccount && (
                      <button className="mt-2" onClick={() => connect()}>
                        <img alt="Wallet" src={walletIcon} className="w-6" />
